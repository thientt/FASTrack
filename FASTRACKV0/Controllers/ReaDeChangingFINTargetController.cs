...
namespace FASTrack.Controllers
{
    ...
    public class ReaDeChangingFINTargetController : AppController
    {
        /// <summary>
        ...
        public async Task<ActionResult> Details(int id)
        {
            ...
            var delayreason = await Rep.SingleAsync(id);
            if (delayreason == null)
                return HttpNotFound();
        ...
        public async Task<ActionResult> Create(DelayReasonViewModel delayreason)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await Rep.AddAsync(data);
        ...
        public async Task<ActionResult> Edit(int id, DelayReasonViewModel delayreason)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await Rep.UpdateAsync(data);
                if (result == Model.SaveResult.SUCCESS)
                    return RedirectToAction("Index");
            }
            return View(delayreason);
        ...
        [Inject]
        public IMSTReasonChangingFINTargetRepository Rep { get; set; }
    }
}
