...
namespace FASTrack.Controllers
{
    ...
    public class DelayReasonController : AppController
    {
        /// <summary>
        ...
        {
            var items = DelayReasonRepository.GetAll();
        ...
        public async Task<ActionResult> Details(int id)
        {
            ...
            var delayreason = await DelayReasonRepository.SingleAsync(id);
            if (delayreason == null)
                return HttpNotFound();
        ...
        public async Task<ActionResult> Create(DelayReasonViewModel delayreason)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await DelayReasonRepository.AddAsync(data);
        ...
        public async Task<ActionResult> Edit(int id, DelayReasonViewModel delayreason)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await DelayReasonRepository.UpdateAsync(data);
                if (result == Model.SaveResult.SUCCESS)
                    return RedirectToAction("Index");
            }
            return View(delayreason);
        ...
        /// </value>
        [Inject]
        public IHSTReasonRepository DelayReasonRepository { get; set; }
    }
}
